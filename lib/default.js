...
function initFlickity(callback) {
	LazyLoad.js('/assets/flickity/flickity.pkgd.min.js',function(){
		$('[data-slideshow]').flickity(slideshowСonfig);
		...
		setTimeout(() => {
			$('[data-card-slideshow]').flickity(cardSlideShowComfig);
		}, 1);
...
function flickityBuild(element,config,callback) {
	try {
		$(element).flickity(config);
	}
	catch(e){
		LazyLoad.js('/assets/flickity/flickity.pkgd.min.js',function(){
			$(element).flickity(config);
...
function initScrollFollow(callback) {
	let filterform_config = {
		limit_elem: '.__cards-flow',
		upper_side: '#header'
	};
	try {
		$('#filters-panel form').simpleScrollFollow(filterform_config);
	}
	catch (e) {
...
function appInit() {
	...
	LazyLoad.js('/assets/jquery-3.2.1/jquery.min.js',function(){
		$('[data-collapsed-text').on('click',function(){
			$(this).removeClass('--collapsed').removeAttr('data-collapsed-text');
		});
		...
		if (pageType == 'item-card') {
			if ($(window).width()>1024) {
				let galleryHeight = document.getElementById('item-gallery').offsetHeight;
				let cardHeight = document.getElementById('item-card').offsetHeight;
				if (galleryHeight < cardHeight) {
					initScrollFollow(()=>{
						$('#item-gallery').simpleScrollFollow({
							limit_elem: '#item-card',
							upper_side: '#header'
						});
					});
				}
					...
					initScrollFollow(()=>{
						$('#item-card').simpleScrollFollow({
							limit_elem: '#item-gallery',
							upper_side: '#header'
						});
					});
				}
		...
		if ($(window).width() > 1024) {
			...
			$(window).on('scroll',function(){
				...
				if (direction == 'down') {
					...
					if (filterFlag == 0 && $bottom.hasClass('--hidden')!=1) {
				...
				if (pageType == 'item-card') {
					if (direction == 'down' && scroll_top > ($('#big-btn').offset().top + $('#big-btn').height()) ) {
						$('#item-btn-bar').addClass('--active');
					}
					else if (direction == 'up') {
						$('#item-btn-bar').removeClass('--active');
					}
				}
			});
		}
